...
Additionally to the ontology gene sets from R package `msigdbr`, we performed another gene enrichment analysis with pathway content from WikiPathways using R package `rWikiPathways`.

```{r}
wp2gene <- rWikiPathways::downloadPathwayArchive(organism="Homo sapiens", format = "gmt") %>% readPathwayGMT
...
```{r}
# Performed pathway enrichment for PRDM1
PRDM1_enrichr_wiki <- enricher(PRDM1_annot$geneId,
                  pAdjustMethod = "BH",
                  qvalueCutoff = 0.05,
                  TERM2GENE=wpid2gene, 
                  TERM2NAME=wpid2name)

PRDM1_enrichr_wikidf <- PRDM1_enrichr_wiki %>%
  filter(p.adjust <= 0.05) %>%
  arrange(desc(Count)) %>%
  select(Description, p.adjust, geneID, Count) %>%
  .@result
...
```{r}
# Performed pathway enrichment for ZNF414
ZNF414_enrichr_wiki <- enricher(ZNF414_annot$geneId,
                  pAdjustMethod = "BH",
                  qvalueCutoff = 0.05,
                  TERM2GENE=wpid2gene, 
                  TERM2NAME=wpid2name)

ZNF414_enrichr_wikidf <- ZNF414_enrichr_wiki %>%
  filter(p.adjust <= 0.05) %>%
  arrange(desc(Count)) %>%
  select(Description, p.adjust, geneID, Count) %>%
  .@result
...
```{r}
# Performed pathway enrichment for overlaps
overlaps_enrichr_wiki <- enricher(overlaps_annot$geneId,
                  pAdjustMethod = "BH",
                  qvalueCutoff = 0.05,
                  TERM2GENE=wpid2gene, 
                  TERM2NAME=wpid2name)

overlaps_enrichr_wikidf <- overlaps_enrichr_wiki %>%
  filter(p.adjust <= 0.05) %>%
  arrange(desc(Count)) %>%
  select(Description, p.adjust, geneID, Count) %>%
  .@result
...
### 4.4) What are pathways/genesets shared in the overlap between PRDM1 and ZNF414 for WikiPathways from R package <code>rWikiPathways</code>?

**Table 5** shows the 13 pathways/genesets shared in the overlap between PRDM1 and ZNF414.

```{r}
datatable(overlaps_enrichr_wikidf, options=list(scrollX = TRUE, scrollY = TRUE), caption = 'Table 5: Pathways/genesets in the overlap between PRDM1 and ZNF414', class = 'cell-border stripe')
```

### 4.5) What pathways differ for WikiPathways from R package <code>rWikiPathways</code>?

**Table 6** shows the 8 pathways that differ between the overlap and PRDM1, i.e. pathways that are in the overlap but not in PRDM1.

```{r}
outersect_enrichr_wiki_PRDM1 <- overlaps_enrichr_wikidf %>%
  rownames_to_column() %>%
  filter(!rowname %in% rownames(PRDM1_enrichr_wikidf)) %>%
  arrange(desc(Count)) %>%
  select(-p.adjust, -Count)

outersect_enrichr_wiki_PRDM1 <- column_to_rownames(outersect_enrichr_wiki_PRDM1, var = "rowname")

datatable(outersect_enrichr_wiki_PRDM1, options=list(scrollX = TRUE, scrollY = TRUE), caption = 'Table 6: Pathways that differ between the overlap and PRDM1', class = 'cell-border stripe')
```

**Table 7** shows the 1 pathway that differ between the overlap and ZNF414, i.e. pathways that are in the overlap but not in ZNF414.

```{r}
outersect_enrichr_wiki_ZNF414 <- overlaps_enrichr_wikidf %>%
  rownames_to_column() %>%
  filter(!rowname %in% rownames(ZNF414_enrichr_wikidf)) %>%
  arrange(desc(Count)) %>%
  select(-p.adjust, -Count)

outersect_enrichr_wiki_ZNF414 <- column_to_rownames(outersect_enrichr_wiki_ZNF414, var = "rowname")

datatable(outersect_enrichr_wiki_ZNF414, options=list(scrollX = TRUE, scrollY = TRUE), caption = 'Table 7: Pathways that differ between the overlap and ZNF414', class = 'cell-border stripe')
```
...
Subsequently, we have a look at the identified pathways of WikiPathways from R package `rWikiPathways`:

- [VEGFA-VEGFR2 Pathway](https://reactome.org/content/detail/R-HSA-4420097) is a pathway for signal transduction in the plasma membrane. It is an important pathway in angiogenesis, the formation of new blood vessels from preexisting vasculature (blood vessels or arrangement of blood vessels in an organ).\

- [Ciliary landscape](https://www.gsea-msigdb.org/gsea/msigdb/geneset_page.jsp?geneSetName=WP_CILIARY_LANDSCAPE) is a pathway involving cilia, cellular projections in eucaryotic cells. Ciliary signalling and proteostasis is important for vesicle transport, the cytoskeleton, signalling and ubiquitination.\

- [Adipogenesis](http://www.gsea-msigdb.org/gsea/msigdb/cards/WP_ADIPOGENESIS) is the formation of adipocytes (fat cells) from stem cells. Adipocytes play a vital role in energy homeostasis and process the largest energy reserve as triglycerol in the body of animals.\

- Pathway [Integrated Breast Cancer Pathway](https://pubchem.ncbi.nlm.nih.gov/pathway/WikiPathways:WP1984) incorporates the most important proteins for Breast Cancer. 

- [mRNA processing](http://www.gsea-msigdb.org/gsea/msigdb/geneset_page.jsp?geneSetName=WP_MRNA_PROCESSING) is the only identified top 5 pathway for WikiPathways that is related to our identified pathways of ontology gene sets. This confirms our assumption that genes in the identified peaks between the overlaps of `PRDM1` and `ZNF414` are indeed important for transcriptional regulation.

Overall, the identified pathways of WikiPathways are related to very diverse biological processes. However, even for these pathways we identified one pathway for transcriptional regulation.
