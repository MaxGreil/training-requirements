...
Additionally to the ontology gene sets from R package `msigdbr`, we performed another gene enrichment analysis. Gene ontology is a very limited pathway annotation that is hard to interpret for many applications. Therefore, we also performed a broad-spectrum [enrichr](https://maayanlab.cloud/Enrichr/) analysis using R package `enrichR`.\

In our enrichr analysis we performed pathway enrichment analysis for all available 172 `enrichR` databases. Finally, the found pathways were filtered to contain only terms with an adjusted p-value smaller than or equal 0.05 and a combined score greater 100.\
...
### 4.4) What are pathways/genesets shared in the overlap between PRDM1 and ZNF414 for pathways from R package <code>enrichR</code>?

**Table 5** shows the 461 pathways/genesets shared in the overlap between PRDM1 and ZNF414.
...
### 4.5) What pathways differ for pathways from R package <code>enrichR</code>?

**Table 6** shows the 171 pathways that differ between the overlap and PRDM1, i.e. pathways that are in the overlap but not in PRDM1.
...
**Table 7** shows the 216 pathways that differ between the overlap and ZNF414, i.e. pathways that are in the overlap but not in ZNF414.
...
Subsequently, we have a look at the identified pathways using the broad-spectrum pathway enrichment analysis with `enrichR`:\

The top 10 identified significant pathways for the overlaps between `PRDM1` and `ZNF414` show pathways of two different databases: `Enrichr_Submissions_TF-Gene_Coocurrence` and `ENCODE_and_ChEA_Consensus_TFs_from_ChIP-X`.\

`Enrichr_Submissions_TF-Gene_Coocurrence` contains 1722 terms, a gene coverage of 12486 genes and	299 genes per term. `ENCODE_and_ChEA_Consensus_TFs_from_ChIP-X` contains 104 terms, a gene coverage of	15562 genes and 887 genes per term.\

**TF** stands for transcription factor. In molecular biology, a TF is a protein that is important for the initiation of RNA polymerase during transcription. **Co-occurence** networks are used to describe potential relationships between entities like in this case TFs. **ENCODE** (ENCyclopedia Of DNA Elements) is a research project aimed at identifying and characterizing all functional elements of the human genome as well as the transcriptome. **ChEA** stands for ChIP Enrichment Analysis. **ChIP-X** describes experiments such as ChIP-chip, ChIP-seq, ChIP-PET and DamID.\

This seems to confirm the results of the above described pathways of ontology gene set that the overlaps between `PRDM1` and `ZNF414` seem to be related to transcriptional regulation.\

Samples from the top 10 `enrichR` pathways are:

- [MNT](https://www.genecards.org/cgi-bin/carddisp.pl?gene=MNT) (MAX Network Transcriptional Repressor) is a Protein Coding gene. Gene Ontology (GO) annotations related to this gene include DNA-binding transcription factor activity and transcription co-activator activity\
- [E2F6](https://www.ncbi.nlm.nih.gov/gene/1876) encodes a member of a family of transcription factors that play a crucial role in the control of the cell cycle
