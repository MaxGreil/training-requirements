...
The data was downloaded via R package `recount`. The data was downloaded via R package `recount` with accession number `SRP055103`. **Table 1** shows the downloaded metadata for study `SRP055103`.
...
We additionally created a dotplot for enrichment result of our GSEA.

```{r}
# Make summary plots
dotplot(gseares) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
IL3Rαhigh- and IL3Rαlow precursor cells can clearly be differentiated based on their gene expression patterns.\ 

The found significant DEGs also show up in a lot of significant pathways during GSEA.
...
All results in this RNA-Seq analysis indicate powerful biological differences between IL3Rαhigh- and IL3Rαlow precursor cells.\

As also stated in the corresponding study this indicated that the gene expression patterns of these cells are likely to be conserved.\

Among the found significant DEGs, genes involved in the differentiation of cell in general, amongst which differentiation of MΦ, OC and antigen presenting cells appeared to be activation increased.
